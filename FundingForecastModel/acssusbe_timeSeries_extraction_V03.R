...
if(dataFrequency == "monthly"){
  ...
  ACSSUSBETXNsDataSeriesSentRecievedSum <- 
    ACSSUSBETXNsDataSeriesSentRecieved[,list(RecievedVolume=sum(RecievedVolume), RecievedValue=sum(RecievedValue), 
  ...
  ACSSUSBETXNsDataSeriesSentRecievedSum[,`:=`(sender="ALLFIS", senderID=9999, 
                                          senderFullName="All Financial Institutions")];
  ##merge created aggregated periodic total volumes with FI level volumes data
  ACSSUSBETXNsDataSeriesSentRecieved <- rbindlist(list(ACSSUSBETXNsDataSeriesSentRecieved, 
                                                       ACSSUSBETXNsDataSeriesSentRecievedSum), 
  ...
  ACSSUSBETXNsDataSeriesSentRecievedSum <- 
    ACSSUSBETXNsDataSeriesSentRecieved[,list(TotalVolume=sum(TotalVolume), TotalValue=sum(TotalValue),
...
##Create a list of time series objects for the FIs as above but with volume
...
ACSSUSBEFITimeSeriesDataTable <- list.names(ACSSUSBEFITimeSeriesDataTable,"ACSSUSBEFITimeSeriesList");##Giving the generated list a name
list.save(ACSSUSBEFITimeSeriesDataTable, ACSSUSBEFITimeSeriesDataTableSavePath);

##Create a list of time series objects for the FIs as above but with Value not volume
ACSSUSBEValueFITimeSeriesDataTable <- 
  getFITimeSeriesDataTable(as.data.table(ACSSUSBETXNsDataSeriesSentRecieved),vectorColumnNames, dataFrequencyColumnName,
                           "Value", "sender",minObservations);
ACSSUSBEValueFITimeSeriesDataTable <- list.names(ACSSUSBEValueFITimeSeriesDataTable,"ACSSUSBEFITimeSeriesList");
list.save(ACSSUSBEValueFITimeSeriesDataTable, ACSSUSBEValueFITimeSeriesDataTableSavePath);


save(ACSSUSBETXNsDataSeriesSentRecieved, file=allFIACSSUSBETXNsDataSeriesSentRecievedSavePath);

write.csv(ACSSUSBETXNsDataSeriesSentRecieved, file=allFIACSSUSBETXNsDataSeriesSentRecievedCSVSavePath);


##Now genereate dummy variables to handle missing USBE data and financial crises 
if(dataFrequency == "monthly"){
