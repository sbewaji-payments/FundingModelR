...
timeSeriesPlotsACSS <- createTSPlots(ACSSFITimeSeriesList, measurementUnit, measurementUnitType, dataFrequency, 
                                     startYear, endYear);
timeSeriesDecompositionPlotsACSS <- 
  createTSDecompositionPlots(ACSSFITimeSeriesList, measurementUnit, measurementUnitType, dataFrequency, startYear, 
...
timeSeriesSARIMAForecastsACSS <- generateSARIMAForecasts(timeSeriesDecompositionACSS, arimaOrder, seasonalOrder, nSPeriods,
                                                        forecastHorizen, arimaConstant, arimaDrift, confidenceLevel);
dtListFILevelSARIMAForecastsACSS <- exportListOfForecastsToExcel(timeSeriesSARIMAForecastsACSS, measurementUnitType, 
                                                                 forecastsSavePath);
dtListFILevelSARIMACoefficientsACSS <- exportListOfForecastModelCoefficientsToExcel(timeSeriesSARIMAForecastsACSS, 
                                                                                    measurementUnitType, forecastsSavePath);
chartListFILevelSARIMAForecastACSS <- 
  exportForecastChartList(timeSeriesARIMAForecastsACSS, dataFrequency, measurementUnit, 
                          measurementUnitType, forecastsSavePath);
gc();
seasonallyAdjustedTimeSeriesPlotsACSS <- 
  createSeasonallyAdjustedTSPlots(ACSSFITimeSeriesList, measurementUnit, measurementUnitType, 
