...
createTSPlots <- function(inputTSList, measUnit, measUnitType, tsFreq, sYr,eYr){
  ...
  while(counter <= length(inputTSList)){
    ...
    tmpPlotList[[counter]] <- list(FinancialInstitution=inputTSList[counter]$LVTSFITimeSeriesList$FinancialInstitution, TimeSeriesChart=tmpPlot);
    counter <- counter+1;
  }
...
createSeasonallyAdjustedTSPlots <- function(inputTSList, measUnit, measUnitType, tsFreq, sYr,eYr, savePath){
  ...
  while(counter <= length(inputTSList)){
    #create the lattice plot
    testSTL <- stl(inputTSList[counter]$LVTSFITimeSeriesList$TimeSeries/measUnit,s.window="periodic");#recalling stl is not efficient 
    ...
    fileLocation <- paste(savePath, inputTSList[counter]$LVTSFITimeSeriesList$FinancialInstitution, measurementUnitTypeSaveString, sep="");
    png(fileLocation);
    ...
    #This has been made to reference the FI name as a primary key to prevent having to loop through every element of the list
    #to pull up a chart for a specific FI later on
    tmpPlotList[[counter]] <- list(FinancialInstitution=inputTSList[counter]$LVTSFITimeSeriesList$FinancialInstitution, SeasonallyAdjustedChart=tmpPlot);
    counter <- counter+1;
...
createTSDecompositionPlots <- function(inputTSList, measUnit, measUnitType, tsFreq, sYr,eYr, savePath){
  ...
  while(counter <= length(inputTSList)){
    ...
    tmpPlot <- xyplot(stl(inputTSList[counter]$LVTSFITimeSeriesList$TimeSeries/measUnit,s.window="periodic", robust=TRUE), 
                      main= paste("Time Series Decomposition for ",list(inputTSList[counter]$LVTSFITimeSeriesList$FinancialInstitution)), 
                      scales=list(cex=0.7, tck=c(1,0), y=list(tick.number=10, rot=360), x=list(tick.number=15)),
                      xlab = paste(tsFreqString), ylab =paste(measurementUnitTypeString, measurementUnitString, sep = " "), na.action = na.omit);
    #add the lattice plot to the list of plots
    #This has been made to reference the FI name as a primary key to prevent having to loop through every element of the list
    #to pull up a chart for a specific FI later on
    tmpPlotList[[counter]] <- 
      list(FinancialInstitution=inputTSList[counter]$LVTSFITimeSeriesList$FinancialInstitution, TimeSeriesDecompositionChart=tmpPlot);
    print(tmpPlot);#printing into the plots display window is required so as to allow the storing of the plot object
    dev.off();#captures the chart in the plots display window and then clears the window
...
createTSPACFPlots <- function(inputTSList, measUnit){
  ...
  while(counter <= length(inputTSList)){
    ...
    #This has been made to reference the FI name as a primary key to prevent having to loop through every element of the list
    #to pull up a chart for a specific FI later on
    tmpPlotList[[counter]] <- list(FinancialInstitution=inputTSList[counter]$LVTSFITimeSeriesList$FinancialInstitution, TimeSeriesPACFChart=tmpPlot);
    counter <- counter + 1;
  }
  return(tmpPlotList) 
}
...
createTSACFPlots <- function(inputTSList, measUnit){
  ...
  while(counter <= length(inputTSList)){
    ...
    #This has been made to reference the FI name as a primary key to prevent having to loop through every element of the list
    #to pull up a chart for a specific FI later on
    tmpPlotList[[counter]] <- list(FinancialInstitution=inputTSList[counter]$LVTSFITimeSeriesList$FinancialInstitution, TimeSeriesACFChart=tmpPlot);
    counter <-  counter + 1;
  }
  return(tmpPlotList) 
}
