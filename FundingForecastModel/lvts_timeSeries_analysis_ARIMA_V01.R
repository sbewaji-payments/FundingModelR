...
timeSeriesARIMAForecastsLVTS <- generateARIMAForecasts(timeSeriesDecompositionLVTS, arimaOrder, forecastHorizen, arimaConstant, arimaDrift, confidenceLevel);
dtListFILevelARIMAForecastsLVTS <- exportListOfForecastsToExcel(timeSeriesARIMAForecastsLVTS, measurementUnitType, forecastsSavePath);
dtListFILevelARIMAForecastErrorsLVTS <- exportListOfForecastErrorsToExcel(timeSeriesARIMAForecastsLVTS, measurementUnitType, forecastsSavePath);
dtListFILevelARIMACoefficientsLVTS <- exportListOfForecastModelCoefficientsToExcel(timeSeriesARIMAForecastsLVTS, measurementUnitType, forecastsSavePath);
chartListFILevelARIMAForecastLVTS <- 
  exportForecastChartList(timeSeriesARIMAForecastsLVTS, dataFrequency, measurementUnit, measurementUnitType, forecastsSavePath);
gc();
