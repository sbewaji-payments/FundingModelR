...
##        1: Included function to compute Seasonal ARIMA(p,d,q)(P,D,Q,n). Lines 390 to 490
...
createTSPlots <- function(inputTSList, measUnit, measUnitType, tsFreq, sYr,eYr){
  ...
  while(counter <= length(inputTSList)){
    ...
    tmpPlot <- xyplot.ts((inputTSList[counter]$ACSSUSBEFITimeSeriesList$TimeSeries/measUnit), 
                         screens = list(inputTSList[counter]$ACSSUSBEFITimeSeriesList$FinancialInstitution),
    ...
    tmpPlotList[[counter]] <- list(FinancialInstitution=inputTSList[counter]$ACSSUSBEFITimeSeriesList$FinancialInstitution, TimeSeriesChart=tmpPlot);
    counter <- counter+1;
  }
...
createSeasonallyAdjustedTSPlots <- function(inputTSList, measUnit, measUnitType, tsFreq, sYr,eYr, savePath){
  ...
  if(measUnitType %in% c("volume","Volume","Vol","VOLUME")){
    ...
    mainTitleLeadingString = "ACSS and USBE Transactions Volume for";
    ...
    mainTitleLeadingString = "ACSS and USBE Transactions Value for";
  }
  ...
  while(counter <= length(inputTSList)){
    #create the lattice plot
    testSTL <- stl((inputTSList[counter]$ACSSUSBEFITimeSeriesList$TimeSeries/measUnit),s.window="periodic");#recalling stl is not efficient 
    ...
    fileLocation <- paste(savePath, inputTSList[counter]$ACSSUSBEFITimeSeriesList$FinancialInstitution, measurementUnitTypeSaveString, sep="");
    png(fileLocation);
    tmpPlot <- xyplot((inputTSList[counter]$ACSSUSBEFITimeSeriesList$TimeSeries/measUnit), 
                      #screens = list(inputTSList[counter]$ACSSUSBEFITimeSeriesList$FinancialInstitution),
                      superpose = TRUE, xlim = extendrange(sYr:eYr),par.settings = graphSettings, 
                      main=paste(mainTitleLeadingString, inputTSList[counter]$ACSSUSBEFITimeSeriesList$FinancialInstitution),
                      scales=list(cex=0.7, tck=c(1,0), y=list(tick.number=10, rot=360), x=list(tick.number=15)), 
                      xlab = paste(tsFreqString), ylab =paste(measurementUnitTypeString, measurementUnitString, sep = " "));
    ...
    #This has been made to reference the FI name as a primary key to prevent having to loop through every element of the list
    #to pull up a chart for a specific FI later on
    tmpPlotList[[counter]] <- list(FinancialInstitution=inputTSList[counter]$ACSSUSBEFITimeSeriesList$FinancialInstitution, SeasonallyAdjustedChart=tmpPlot);
    counter <- counter+1;
...
createTSDecompositionPlots <- function(inputTSList, measUnit, measUnitType, tsFreq, sYr,eYr, savePath){
  ...
  while(counter <= length(inputTSList)){
    ...
    png(fileLocation);
    #create the lattice plot
    tmpPlot <- xyplot(stl((inputTSList[counter]$ACSSUSBEFITimeSeriesList$TimeSeries/measUnit),s.window="periodic", robust=TRUE), 
                      main= paste("Time Series Decomposition for ",list(inputTSList[counter]$ACSSUSBEFITimeSeriesList$FinancialInstitution)), 
                      scales=list(cex=0.7, tck=c(1,0), y=list(tick.number=10, rot=360), x=list(tick.number=15)),
                      xlab = paste(tsFreqString), ylab =paste(measurementUnitTypeString, measurementUnitString, sep = " "));
    #add the lattice plot to the list of plots  + xlim(c(xAxisMin,xAxisMax)
    #This has been made to reference the FI name as a primary key to prevent having to loop through every element of the list
    #to pull up a chart for a specific FI later on
    tmpPlotList[[counter]] <- 
      list(FinancialInstitution=inputTSList[counter]$ACSSUSBEFITimeSeriesList$FinancialInstitution, TimeSeriesDecompositionChart=tmpPlot);
    print(tmpPlot);#printing into the plots display window is required so as to allow the storing of the plot object
    dev.off();#captures the chart in the plots display window and then clears the window
...
createTSPACFPlots <- function(inputTSList, measUnit){
  ...
  while(counter <= length(inputTSList)){
    ...
    tmpPlot <- pacf((inputTSList[counter]$ACSSUSBEFITimeSeriesList$TimeSeries/measUnit),
                    main= paste("PACF of ", list(inputTSList[counter]$ACSSUSBEFITimeSeriesList$FinancialInstitution)));
    #add the lattice plot to the list of plots
    #This has been made to reference the FI name as a primary key to prevent having to loop through every element of the list
    #to pull up a chart for a specific FI later on
    tmpPlotList[[counter]] <- list(FinancialInstitution=inputTSList[counter]$ACSSUSBEFITimeSeriesList$FinancialInstitution, TimeSeriesPACFChart=tmpPlot);
    counter <- counter + 1;
  }
  return(tmpPlotList) 
}
...
createTSACFPlots <- function(inputTSList, measUnit){
  ...
  while(counter <= length(inputTSList)){
    ...
    tmpPlot <- Acf((inputTSList[counter]$ACSSUSBEFITimeSeriesList$TimeSeries/measUnit),
                   main= paste("ACF of ", list(inputTSList[counter]$ACSSUSBEFITimeSeriesList$FinancialInstitution)));
    #add the lattice plot to the list of plots
    #This has been made to reference the FI name as a primary key to prevent having to loop through every element of the list
    #to pull up a chart for a specific FI later on
    tmpPlotList[[counter]] <- list(FinancialInstitution=inputTSList[counter]$ACSSUSBEFITimeSeriesList$FinancialInstitution, TimeSeriesACFChart=tmpPlot);
    counter <-  counter + 1;
  }
  return(tmpPlotList) 
}
...
createTSDecomposition <- function(inputTSList, measUnit){
  ...
  while(counter <= length(inputTSList)){
    ...
    tsDecom <- stl((inputTSList[counter]$ACSSUSBEFITimeSeriesList$TimeSeries/measUnit),s.window="periodic", robust=TRUE);
    
    #add the lattice plot to the list of plots
    tsDecomList[[counter]] <- list(FinancialInstitution=inputTSList[counter]$ACSSUSBEFITimeSeriesList$FinancialInstitution, TimeSeriesDecomposition=tsDecom);
    counter <- counter+1;
  }
  return(tsDecomList); 
}
...
generateARIMAForecasts <- function(inputTSDecompList, inputArimaOrder, fcstHrzn, cnst, drft, confLev){ ##browser()
  ...
  while(counter <= length(inputTSDecompList)){
    ...
    fcstValues <- fcasting;
    fcstErrors <- residuals(fcasting);
    meanSqrErrors <- mean(fcstErrors^2);
    bjTest <- Box.test(fcstErrors, lag=16, fitdf=4, type="Ljung-Box");
    fcastSummary <- summary(fcasting);
...
exportListOfForecastsToExcel <- function(inputForecastsList, measUnitType, savePath){
  ...
  while(counter<=length(inputForecastsList)){
    #list(FinancialInstitution=i, TimeSeries=tempTS);
    dtList[[counter]] <- list(FinancialInstitution=inputForecastsList[[counter]]$FinancialInstitution,
                              as.data.table(inputForecastsList[[counter]]$ForecastedSeries));
    # dtList[[counter]] <- list(FinancialInstitution=inputForecastsList[[counter]]$FinancialInstitution, 
    #                           ForecastedSeries=as.data.table(inputForecastsList[[counter]]$ForecastedSeries));
...
exportListOfForecastErrorsToExcel <- function(inputForecastsList, measUnitType, savePath){
  ...
  while(counter<=length(inputForecastsList)){
    dtList[[counter]] <- list(FinancialInstitution=inputForecastsList[[counter]]$FinancialInstitution, 
                              ForecastErrors=as.data.table(inputForecastsList[[counter]]$ForecastErrors));
