...
##        1: Included function to compute Seasonal ARIMA(p,d,q)(P,D,Q,n). Lines 378 to 478
...
createTSPlots <- function(inputTSList, measUnit, measUnitType, tsFreq, sYr,eYr){
  ...
  while(counter <= length(inputTSList)){
    ...
    tmpPlot <- xyplot.ts(inputTSList[counter]$ACSSFITimeSeriesList$TimeSeries/measUnit, 
                         screens = list(inputTSList[counter]$ACSSFITimeSeriesList$FinancialInstitution),
    ...
    tmpPlotList[[counter]] <- list(FinancialInstitution=inputTSList[counter]$ACSSFITimeSeriesList$FinancialInstitution, TimeSeriesChart=tmpPlot);
    counter <- counter+1;
  }
...
createSeasonallyAdjustedTSPlots <- function(inputTSList, measUnit, measUnitType, tsFreq, sYr,eYr, savePath){
  ...
  if(measUnitType %in% c("volume","Volume","Vol","VOLUME")){
    ...
    mainTitleLeadingString = "ACSS Transactions Volume for";
    ...
    mainTitleLeadingString = "ACSS Transactions Value for";
  }
  ...
  while(counter <= length(inputTSList)){
    #create the lattice plot
    testSTL <- stl(inputTSList[counter]$ACSSFITimeSeriesList$TimeSeries/measUnit,s.window="periodic");#recalling stl is not efficient 
    ...
    fileLocation <- paste(savePath, inputTSList[counter]$ACSSFITimeSeriesList$FinancialInstitution, measurementUnitTypeSaveString, sep="");
    png(fileLocation);
    tmpPlot <- xyplot(inputTSList[counter]$ACSSFITimeSeriesList$TimeSeries/measUnit, 
                         #screens = list(inputTSList[counter]$ACSSFITimeSeriesList$FinancialInstitution),
                         superpose = TRUE, xlim = extendrange(sYr:eYr),par.settings = graphSettings, 
                         main=paste(mainTitleLeadingString, inputTSList[counter]$ACSSFITimeSeriesList$FinancialInstitution),
                         scales=list(cex=0.7, tck=c(1,0), y=list(tick.number=10, rot=360), x=list(tick.number=15)), 
                         xlab = paste(tsFreqString), ylab =paste(measurementUnitTypeString, measurementUnitString, sep = " "));
    ...
    #This has been made to reference the FI name as a primary key to prevent having to loop through every element of the list
    #to pull up a chart for a specific FI later on
    tmpPlotList[[counter]] <- list(FinancialInstitution=inputTSList[counter]$ACSSFITimeSeriesList$FinancialInstitution, SeasonallyAdjustedChart=tmpPlot);
    counter <- counter+1;
...
createTSDecompositionPlots <- function(inputTSList, measUnit, measUnitType, tsFreq, sYr,eYr, savePath){
  ...
  while(counter <= length(inputTSList)){
    ...
    png(fileLocation);
    #create the lattice plot
    tmpPlot <- xyplot(stl(inputTSList[counter]$ACSSFITimeSeriesList$TimeSeries/measUnit,s.window="periodic", robust=TRUE), 
                      main= paste("Time Series Decomposition for ",list(inputTSList[counter]$ACSSFITimeSeriesList$FinancialInstitution)), 
                      scales=list(cex=0.7, tck=c(1,0), y=list(tick.number=10, rot=360), x=list(tick.number=15)),
                      xlab = paste(tsFreqString), ylab =paste(measurementUnitTypeString, measurementUnitString, sep = " "));
    #add the lattice plot to the list of plots
    #This has been made to reference the FI name as a primary key to prevent having to loop through every element of the list
    #to pull up a chart for a specific FI later on
    tmpPlotList[[counter]] <- 
      list(FinancialInstitution=inputTSList[counter]$ACSSFITimeSeriesList$FinancialInstitution, TimeSeriesDecompositionChart=tmpPlot);
    print(tmpPlot);#printing into the plots display window is required so as to allow the storing of the plot object
    dev.off();#captures the chart in the plots display window and then clears the window
...
createTSPACFPlots <- function(inputTSList, measUnit){
  ...
  while(counter <= length(inputTSList)){
    ...
    tmpPlot <- pacf(inputTSList[counter]$ACSSFITimeSeriesList$TimeSeries/measUnit,
                    main= paste("PACF of ", list(inputTSList[counter]$ACSSFITimeSeriesList$FinancialInstitution)));
    #add the lattice plot to the list of plots
    #This has been made to reference the FI name as a primary key to prevent having to loop through every element of the list
    #to pull up a chart for a specific FI later on
    tmpPlotList[[counter]] <- list(FinancialInstitution=inputTSList[counter]$ACSSFITimeSeriesList$FinancialInstitution, TimeSeriesPACFChart=tmpPlot);
    counter <- counter + 1;
  }
  return(tmpPlotList) 
}
...
createTSACFPlots <- function(inputTSList, measUnit){
  ...
  while(counter <= length(inputTSList)){
    ...
    tmpPlot <- Acf(inputTSList[counter]$ACSSFITimeSeriesList$TimeSeries/measUnit,
                    main= paste("ACF of ", list(inputTSList[counter]$ACSSFITimeSeriesList$FinancialInstitution)));
    #add the lattice plot to the list of plots
    #This has been made to reference the FI name as a primary key to prevent having to loop through every element of the list
    #to pull up a chart for a specific FI later on
    tmpPlotList[[counter]] <- list(FinancialInstitution=inputTSList[counter]$ACSSFITimeSeriesList$FinancialInstitution, TimeSeriesACFChart=tmpPlot);
    counter <-  counter + 1;
  }
  return(tmpPlotList) 
}
...
createTSDecomposition <- function(inputTSList, measUnit){
  ...
  while(counter <= length(inputTSList)){
    ...
    tsDecom <- stl(inputTSList[counter]$ACSSFITimeSeriesList$TimeSeries/measUnit,s.window="periodic", robust=TRUE);
    
    #add the lattice plot to the list of plots
    tsDecomList[[counter]] <- list(FinancialInstitution=inputTSList[counter]$ACSSFITimeSeriesList$FinancialInstitution, TimeSeriesDecomposition=tsDecom);
    counter <- counter+1;
  }
  return(tsDecomList); 
}
...
generateARIMAForecasts <- function(inputTSDecompList, inputArimaOrder, fcstHrzn, cnst, drft, confLev){
  #declare local variables
  tmpSTL <- NULL;
  fcasting <- NULL;
  tsfcastList <- NULL;
...
exportForecastChartList <- function(inputForecastsList, dataFrequency, measUnit, measUnitType, savePath){
  ...
  if(measUnitType %in% c("volume","Volume","Vol","VOLUME")){
    ...
    mainTitleLeadingString = "ACSS Transactions Volume Forecast for";
  } else if(measUnitType %in% c("value","Value","Val","VALUE")){
    measurementUnitTypeString = "Transactions Value";
    measurementUnitTypeSaveString = "TransactionsValueForecast.png";
    mainTitleLeadingString = "ACSS Transactions Value Forecast for";
  }
